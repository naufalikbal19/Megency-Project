...
NgecekCiter(playerid)
{
	if(gettime() > pData[playerid][pACTime])
	{
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && GetVehicleSpeed(GetPlayerVehicleID(playerid)) > 350 && pData[playerid][pAdmin] < 2)
		{
		    ...
		    {
                new dc[128];
                SendStaffMessage(COLOR_YELLOW, ""RED_E"%s"WHITE_E"[%i] is possibly speedhacking, speed: %.1f km/h.", ReturnName(playerid), playerid, GetVehicleSpeed(GetPlayerVehicleID(playerid)));
			...
			{
                new dc[128];
                SendStaffMessage(COLOR_RED, "[ANTICHEAT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Speed hacks", ReturnName(playerid), SERVER_BOT);
			...
			GetPlayerArmour(playerid, armor);
  			...
  			if(!(gettime() - pData[playerid][pLastUpdate] > 5))
  			{
				if(floatround(armor) > floatround(pData[playerid][pArmour]) && gettime() > pData[playerid][pACTime] && gettime() > pData[playerid][pArmorTime] && pData[playerid][pAdmin] < 2)
				{
				    ...
				    {
                        new dc[128];
                        SendStaffMessage(COLOR_YELLOW, ""RED_E"%s"WHITE_E"[%i] is possibly Armor hacks, (old: %.2f, new: %.2f)", ReturnName(playerid), playerid, pData[playerid][pArmour], armor);
					...
					{
                        new dc[128];
                        SendStaffMessage(COLOR_RED, "[ANTICHEAT]: %s"WHITE_E" telah Mengunakan Armor Vest %s", ReturnName(playerid), SERVER_BOT);
                        format(dc, sizeof(dc),  "```\n<!> %s cek playernya min,kalo armornya lebih dari 200 berarti armor heck %s\nAlasan:Anti Cheat Armor heck lagi bug okawokao [pake armor vest! bukan armor cheat,```", ReturnName(playerid), SERVER_BOT);
                        SendDiscordMessage(3, dc);
					}
				}
			}
