 	...
 	if(rows)
  	{
		for(new z = 0; z < rows; z++)
		{
			...
			cache_get_value_name_int(z, "tembaga", vsData[vehid][vsTembaga]);
			cache_get_value_name_int(z, "emas", vsData[vehid][vsEmas]);
			cache_get_value_name_int(z, "aluminium", vsData[vehid][vsAluminium]);
...
Vehicle_StorageSave(vehicleid)
{
	...
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`tembaga`= %d,", cQuery, vsData[x][vsTembaga]);
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`emas`= %d,", cQuery, vsData[x][vsEmas]);
	mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`aluminium`= %d,", cQuery, vsData[x][vsAluminium]);
