...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	{		
		...
		if(pData[playerid][pPassword] > 10)
		{
			...
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login to Shine Green Roleplay", lstring, "Login", "Quit");
		}
...
function RefullCar(playerid, vehicleid)
{
	...
	if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
    {
		if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
		{
			...
			SetVehicleFuel(vehicleid, fuels+100);
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1753.041748, -1861.587890, 13.577333);
		SetPlayerFacingAngle(playerid, 358.350769);
...
function UpdateNametag(playerid)
{
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
    {
        ...
        {
            new nametag[128];
            GetPlayerArmour(i, pData[playerid][pArmour]);
            GetPlayerHealth(i, pData[playerid][pHealth]);
            //GetPlayerName(i, playername, sizeof(playername));
            if(pData[i][pMaskOn] == 1)
            {
                format(nametag, sizeof(nametag), "Mask_#%d\n"WHITE_E"HP: ["RED_E"%.1f"WHITE_E"] | AR: ["RED_E"%.1f"WHITE_E"]", pData[i][pMaskID], pData[i][pHealth], pData[i][pArmour]); 
            }
            else
            {
                format(nametag, sizeof(nametag), "ID [%i]\n"WHITE_E"HP: ["RED_E"%.1f"WHITE_E"] | AR: ["RED_E"%.1f"WHITE_E"]", i, pData[i][pHealth], pData[i][pArmour]);
            }
