...
CMD:editdealer(playerid, params[])
{
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", dtype))
            return Usage(playerid, "/editbisnis [id] [Type] [1.Motorcycle 2.Cars 3.Unique Cars 4.Job Cars 5.Truck 6.kapal laut]");
...
CMD:createdealer(playerid, params[])
{
	...
	if(sscanf(params, "dd", price, type))
		return Usage(playerid, "/createdealer [price] [type 1.Motorcycle 2.Cars 3.Unique Cars 4. Jobs Vehicle 5. Truck 6.dealer kapal]");
...
DealerBuyVehicle(playerid, dealerid)
{
    ...
    switch(DealerData[dealerid][dealerType])
    {
		...
		case 6:
		{
			new str[1024];
			format(str, sizeof(str), ""WHITE_E"%s\t\t{3498DB}%s\n%s\t\t{3498DB}%s\n%s\t{3498DB}%s\n%s\t{3498DB}%s\n%s\t{3498DB}%s\n%s\t{3498DB}%s\n",
			GetVehicleModelName(454), FormatMoney(GetDealerVehicleCost(454)),
			...
			ShowPlayerDialog(playerid, DIALOG_BUYKAPALVEHICLE, DIALOG_STYLE_LIST, "Kapal DealerShip", str, "Buy", "Close");
		}
    }
    return 1;
}
...
GetDealerVehicleCost(carid)
{
	...
	//CATEGORY KAPAL LAUT
	if(carid == 454) return 40000; //TROPIC ANJAY
	if(carid == 446) return 50000; //SQALAO ANJAY
	if(carid == 452) return 70000; //SPEEDER ANJAY
	if(carid == 453) return 30000; //REEFFER ANJAY
	if(carid == 493) return 95000; //JEETMAx ANJAY
	if(carid == 484) return 75000; //MARQUIZ
 	return -1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_BUYDEALERCARS_CONFIRM_M, DIALOG_STYLE_MSGBOX, "Private Vehicles", tstr, "Buy", "Batal");
				}
				case 1:
				{
					new modelid = 509;
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_RENT_BIKECONFIRM, DIALOG_STYLE_MSGBOX, "Private Vehicles", tstr, "Buy", "Batal");
				}
				case 1:
				{
					new modelid = 403;
	...
	if(dialogid == DIALOG_BUYKAPALVEHICLE)
	{
		if(response)
		{
			switch(listitem)
