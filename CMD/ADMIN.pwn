...
CMD:acmds(playerid)
{
	...
	if(pData[playerid][pAdmin] >= 1)
	{
		...
		strcat(line3, "{ff0000}[Volunteer]: {ffffff}/destroysapd, /destroysags, /destroysamd, /destroysana, /respawndmv, /respawnjobs, /setcs\n\n");
	}
	...
	strcat(line3, "\n"BLUE_E"MIDLEST:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"MIDLEST:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
{
	if(pData[playerid][pAdmin] < 2)
		return Error(playerid, "Cmd Ini Khusus Admin Ya kontol");
	{
...
CMD:adamage(playerid, params[])
{
	...
	new otherid;
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/adamage [playerid/PartOfName]");
	...
	DisplayDamage(playerid, otherid);
	return 1;
}
...
CMD:destroysapd(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] < 3)
			return PermissionError(playerid);
			
	for(new x;x<sizeof(SAPDVeh);x++)
	{
		if(IsVehicleEmpty(SAPDVeh[x]))
		{
...
CMD:destroysags(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] < 3)
			return PermissionError(playerid);
			
	for(new x;x<sizeof(SAGSVeh);x++)
	{
		if(IsVehicleEmpty(SAGSVeh[x]))
		{
			DestroyVehicle(SAGSVeh[x]);
		}
	}
...
CMD:destroysamd(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] < 3)
			return PermissionError(playerid);
			
	for(new x;x<sizeof(SAMDVeh);x++)
	{
		if(IsVehicleEmpty(SAMDVeh[x]))
		{
			DestroyVehicle(SAMDVeh[x]);
		}
	}
...
CMD:destroysana(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] < 3)
			return PermissionError(playerid);
			
	for(new x;x<sizeof(SANAVeh);x++)
	{
		if(IsVehicleEmpty(SANAVeh[x]))
		{
			DestroyVehicle(SANAVeh[x]);
		}
	}
...
CMD:setstres(playerid, params[])
{
	...
	new watehel, stresid;
	if(sscanf(params, "ud", stresid, watehel))
		return Usage(playerid, "/setstres [playerid] <Jumlahnya>");

	if(!IsPlayerConnected(stresid))
		return Error(playerid, "player blom masuk kontol");

	pData[stresid][pBladder] = watehel;
	SendStaffMessage(COLOR_YELLOW, "Admin %s Telah Menset Jumlah Stresnya si %s. Jahat Banget Admin %s bikin orang stres", pData[playerid][pAdminname], pData[stresid][pName], pData[playerid][pAdminname]);
    Servers(stresid, "Admin %s Telah Menset Bar Stres Anda", pData[playerid][pAdminname]);
    return 1;
}
//----------------------------[ Admin Level 4 ]---------------
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW 5.Pedagang] [rank 1-13]");
...
CMD:setleader(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid, fid))
        return Usage(playerid, "/setleader [playerid/PartOfName] [0.None, 1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW, 5.PDG");
...
CMD:nitrobos(playerid, params[])
{
	...
	SendStaffMessage(COLOR_YELLOW, "%s Kamu Berhasil Menambahkan Nitro Kepada Mobil Kamu", pData[playerid][pAdminname]);
	return 1;
}
CMD:veh(playerid, params[])
{
    ...
    SetVehicleNumberPlate(vehicleid, "KONTOL");
...
CMD:setadminlevel(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Player belum masuk!");
	if(alevel > 7)
		return Error(playerid, "Level can't be higher than 7!");
	if(alevel < 0)
		return Error(playerid, "Level can't be lower than 0!");
...
CMD:setstat(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]S()[32]", otherid, name, param))
		{
			...
			Info(playerid, "Names: Wheat, Orange, Bandage, Medkit, Medicine, Marijuana, Component, Material, tembaga");
			return 1;
		}
		...
		else if(!strcmp(name, "tembaga", true)) 
		{			
			if(sscanf(param, "i", ammount))
			{
				return Usage(playerid, "/setstat [playerid] [tembaga] [ammount]");
			}

			pData[otherid][bTembaga] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Tembaga {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Tembaga{ffffff} kamu menjadi %d.", pData[playerid][pAdminname], ammount);

			mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET tembaga = %i WHERE reg_id = %i", pData[otherid][bTembaga], pData[otherid][pID]);
	    	mysql_tquery(g_SQL, query);
		}
		else 
		{
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FF0000}Moderator Commands | Shine Green Roleplay", string, "Close","");
	return 1;
}
