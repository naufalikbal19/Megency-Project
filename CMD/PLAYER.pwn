...
CMD:help(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_HELP, DIALOG_STYLE_LIST, "SGRP Help Menu", info, "Select", "Close");
	return 1;
}
...
CMD:fixvisu(playerid, params[])
{
   ...
   Info(playerid, "Visual Anda Kembali Normal Tchuy");
   return 1;
}
CMD:gendong(playerid, params[])
{
    new yangdigendong, string[512], string2[512];
    if(sscanf(params, "u", yangdigendong)) return SendClientMessage(playerid, COLOR_WHITE, "/carry [playerid] NOTE: Jika ingin letakkan /drag [playerid] lagi.");
    if(GetDistanceBetweenPlayers(playerid, yangdigendong) < 2)
    {
        if(Targetgendong[yangdigendong] == 0 && Yangmenggendong[playerid] == 0)
        {
            Targetgendong[yangdigendong] = 1;
            Yangmenggendong[playerid] = 1;
            format(string, sizeof(string), "Anda digendong oleh {FF0000}%s.",(playerid));
            format(string2, sizeof(string2), "Anda menggendong {FF0000}%s.",(yangdigendong));
            ApplyAnimation(yangdigendong, "FAT", "IDLE_tired", 4.1, 0, 1, 1, 1, 0, 1);
            gendongtimer[yangdigendong] = SetTimerEx("Draggingt", 1000, 1, "dd", playerid,yangdigendong);
        }
        else if(Targetgendong[yangdigendong] == 1 && Yangmenggendong[playerid] == 1)
        {
            Targetgendong[yangdigendong] = 0;
            Yangmenggendong[playerid] = 0;
            Info(playerid, "Anda berhenti menggendong.");
            Info(yangdigendong, "Anda tidak digendong lagi.");
            ClearAnimations(playerid);
            KillTimer(gendongtimer[yangdigendong]);
        }
    }
    else
    {
        Info(playerid, "Anda tidak dekat dengan pemain tersebut.");
    }
    return 1;
}

CMD:gendong2(playerid, params[])
{
    new yangdigendong;
    if(GetDistanceBetweenPlayers(playerid, yangdigendong) < 2)
    {
        if(Targetgendong[yangdigendong] == 1 && Yangmenggendong[playerid] == 1)
        {
            Targetgendong[yangdigendong] = 0;
            Yangmenggendong[playerid] = 0;
            ClearAnimations(playerid);
        ...
        {
        	Info(playerid, "Anda tidak sedang digendong.");
		}
    }
    return 1;
}
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Coder: "YELLOW_E"Vincent Guardie for Update\n");
	strcat(line3, ""LB_E"Support Mapper: "YELLOW_E"Vincent and his Friends, Mahes\n");
	strcat(line3, ""LB_E"Support Website: "YELLOW_E"COMINGSON\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"GREEN_E"Terima kasih telah bergabung dengan kami! Copyright © 2023 | Shine Green Roleplay.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"SHINEGREEN:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
CMD:putincar(playerid, params[])
{
    ...
    if(sscanf(params, "u", otherid))
        return Usage(playerid, "/putincar [playerid/PartOfName]");
    ...
    if(!pData[otherid][pInjured])
        return Error(playerid, "Player Ini Tidak Dalam Keadaan Pingsan.");

    if(vehicleid == INVALID_VEHICLE_ID)
        return Error(playerid, "You are not near any vehicle.");

    if(GetVehicleMaxSeats(vehicleid) < 2)
        return Error(playerid, "Mobil lu Cuman Ada 2 Pintu Anying.Butuh 4 Pintu");
...
CMD:vip(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"SHINEGREEN:RP "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
{
    new line3[3500];
    strcat(line3, ""RED_E"...:::... "DOOM_"Donate List MONIA Roleplay "RED_E"...:::...\n");
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"MONIA: "WHITE_E"DONATE LIST", line3, "Okay", "");
	return 1;
}*/
...
CMD:damagelogs(playerid, params[])
{
	...
	DisplayDamage(playerid, playerid);
	return 1;
}
...
CMD:death(playerid, params[])
{
    ...
    if((gettime()-GetPVarInt(playerid, "GiveUptime")) < 420)
        return Error(playerid, "Kamu harus menunggu 7 menit untuk kembali kerumah sakit");
...
CMD:time(playerid)
{
	...
	format(line2, sizeof(line2), ""WHITE_E"Paycheck Time: "YELLOW_E"%d remaining\n"WHITE_E"Delay Job: "RED_E"%d Detik\n"WHITE_E"Delay Side Job: "RED_E"%d Detik\n"WHITE_E"Plant Time(Farmer): "RED_E"%d Detik\n"WHITE_E"Arrest Time: "RED_E"%d Detik\n"WHITE_E"Jail Time: "RED_E"%d Detik\n", paycheck, pData[playerid][pJobTime], pData[playerid][pSideJobTime], pData[playerid][pPlantTime], pData[playerid][pArrestTime], pData[playerid][pJailTime]);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"SGRP RP: "WHITE_E"Time", line2, "Oke", "");
	return 1;
}
...
CMD:claimpv(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1296.0533, -1264.1348, 13.5939)) return Error(playerid, "Kamu harus berada di Kantor Insurance!");
	new carid;
	if(sscanf(params, "d", carid)) return SendClientMessage(playerid, -1, "[usage] /claimpv [vehicleid]");
	foreach(new i : PVehicles)
	{
		if(pvData[i][cID] == carid)
	...
	if(pData[playerid][pVip] > 0)
	{
		if(pData[playerid][pJob] == 0 || pData[playerid][pJob2] == 0)
		{
			if(pData[playerid][pJob] == 0)
			{
				...
				else if(pData[playerid][pJob] == 0 && GetPlayerState(playerid) == 1 && IsPlayerInRangeOfPoint(playerid, 3.0, -419.352722, -1760.601318, 5.828370))
				{
...
CMD:mask(playerid, params[])
{
	...
	switch (pData[playerid][pMaskOn])
    {
        ...
        {
			new str[128];
			new Float:health = GetPlayerHealth(playerid, health), Float:armor = GetPlayerArmour(playerid, armor);
			...
			if(!IsValidDynamic3DTextLabel(pData[playerid][pMaskLabel]))
				pData[playerid][pMaskLabel] = CreateDynamic3DTextLabel(str, COLOR_WHITE, 0.0, 0.0, 0.2, 8.0, playerid, INVALID_VEHICLE_ID, 0, -1, -1);
...
CMD:phone(playerid, params[])
{
	...
	for(new i = 0; i < 29; i++) {
		PlayerTextDrawShow(playerid, TDHPV2[playerid][i]);
	}
	PlayerTextDrawShow(playerid, KLIKKEMBALI[playerid]);
	PlayerTextDrawShow(playerid, KLIKSMS[playerid]);
	PlayerTextDrawShow(playerid, KLIKGPS[playerid]);
	PlayerTextDrawShow(playerid, KLIKRUMAH[playerid]);
	PlayerTextDrawShow(playerid, KLIKTW[playerid]);
	PlayerTextDrawShow(playerid, KLIKSHARELOC[playerid]);
	PlayerTextDrawShow(playerid, KLIKSETTING[playerid]);
	PlayerTextDrawShow(playerid, KLIKCALL[playerid]);
	PlayerTextDrawShow(playerid, KLIKPS[playerid]);
	PlayerTextDrawShow(playerid, KLIKKONTAK[playerid]);
	SelectTextDraw(playerid, COLOR_LBLUE);
	return 1;
}
...
CMD:buy(playerid, params[])
{
	...
	//Drugs
	if(IsPlayerInRangeOfPoint(playerid, 2.5, -1635.008178, -2235.539794, 31.476562))
	{
...
CMD:sp(playerid, params[])
{
	...
	pData[playerid][pSnack] += 15;
	pData[playerid][pSprunk] += 15;
	InfoTD_MSG(playerid, 7000, "~g~Money +1000, 1 Hape, Snack +15, Sprunk +15");
