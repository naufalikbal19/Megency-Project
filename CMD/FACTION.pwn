...
CMD:od(playerid, params[])
{
	...
	for(new fid = 1; fid < 5; fid++)
	{
		if(pData[playerid][pFaction] == 1) {
			...
			SendFactionMessage(fid, 0xfff200AA, "** (( %s: %s ))", pData[playerid][pName], text);
			//format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
			//SetPlayerChatBubble(playerid, mstr, 0xFFD7004A, 10.0, 3000);
		}
		else if(pData[playerid][pFaction] == 3) {
...
CMD:locker(playerid, params[])
{
	...
	foreach(new lid : Lockers)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]))
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_LOCKERPEDAGANG, DIALOG_STYLE_LIST, "PDG Lockers", "Toggle Duty\nHealth\nArmour", "Proceed", "Cancel");
			}
...
{
    new carid;
    if(sscanf(params, "d", carid)) return SendClientMessage(playerid, -1, "[USAGE] /impound vehicleid");

    if(pData[playerid][pFaction] != 1)
        return Error(playerid, "Kamu harus menjadi police officer.");

    foreach(new i : PVehicles){
        if(pvData[i][cVeh] == carid){
            new Float:x, Float:y, Float:z;
            GetVehiclePos(pvData[i][cVeh], x,y,z);
            if(IsPlayerInRangeOfPoint(playerid, 2.5, x,y,z)) return SendClientMessage(playerid, -1, "{ff0000}ERROR:{ffffff} You must near with that vehicle.");

            pvData[i][cClaim] = 1;
            pvData[i][cClaimTime] = gettime() + (1 * 600);
            foreach(new pid : Player) if (pvData[i][cOwner] == pData[pid][pID])
                {
                    Info(pid, "Kendaraan anda dimasukan ke impound oleh pihak kepolisian,silahkan ambil di asuransi");
                }
            ...
            Info(playerid, "Kendaraan Model %s(%d)  ke impound.", GetVehicleModelName(pvData[i][cModel]), pvData[i][cVeh]);
        }
    }
    return 1;
}
...
CMD:impound(playerid, params[])
{
	...
	{	
		...
		{
			pvData[carid][cClaim] = 1;
			pvData[carid][cClaimTime] = gettime() + (1 * 600);
			...
			Info(playerid, "Kendaraan Model %s(%d) dimasukan ke impound.", GetVehicleModelName(pvData[carid][cModel]), pvData[carid][cVeh]);
		}
	}	
	else return Error(playerid, "Tidak Ada Kendaraan Di Sekitar Anda");
	return 1;
}


CMD:projekp(playerid, params[])
{
	new carid = -1, vehid;
    ...
    if(pData[playerid][pFaction] !=1)
    	return Error(playerid, "Kamu Bukan SAPD");
    ...
    if((carid = Vehicle_Nearest2(playerid)) != -1)
    {
        ...
        Info(playerid, "Kendaraan Model %s(%d) dimasukan ke impound.", GetVehicleModelName(pvData[carid][cModel]), pvData[carid][cVeh]);
        Info(carid, "Kendaraan Kamu Telah Di Impound Oleh Pihak Kepolisian,Silahkan Ambil Di Asuransi");
    }
    return 1;
}*/
...
CMD:menumasak(playerid, params[])
{
	...
	format(Dstring, sizeof(Dstring), "{ffffff}%sPizza (5)\tpotato 2 + snack 2\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sCoca-Cola(5)\tSprunk 2 + Air Mineral 2\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sFried Chiken(5)\tPotato 2 + Ayam Filet 2\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sNasi Padang(5)\tOrange 3 + Potato 3 + Ayam Filet 1\n", Dstring);
...
function Menumasak(playerid, gunid, ammo)
{
	...
	if(pData[playerid][pActivityTime] >= 100)
	{
		InfoTD_MSG(playerid, 8000, "Makanan Sudah Matang");
		TogglePlayerControllable(playerid, 1);
...
ShowPedagangMenu(playerid, pid)
{
    ...
    new str[256], vstr[64];
    format(vstr, sizeof vstr,"gudang pedagang");
    ...
    ShowPlayerDialog(playerid, PEDAGANG_MENU, DIALOG_STYLE_LIST, vstr, str, "Select", "Cancel");
